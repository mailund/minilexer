...
parse_f <- function() {
  ...
  # keep consuming tokens as long as they are `numbers`
  values <- c()
  while (identical(stream$current_type(), 'number')) {
...
parse_v <- function() {
  ...
  if (length(values) == 4) {
    ...
    message <- glue("Expecting 3 or 4 values only for vertex, but got {length(values)} starting at position {start_position}")
    stop(message)
  }
  as.numeric(values)
}
